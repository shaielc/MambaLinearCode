...
def mask_larger_matrix(A, mask):
    A[:, :mask.size(0), :mask.size(1)][mask.unsqueeze(0).expand(A.size(0),mask.size(0),mask.size(1))] = 0.0
...
class ECCMLayer(nn.Module):
    def __init__(
        ...
        self.register_buffer('pc_mask', build_mask(code))
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            assert self.activation in ["silu", "swish"]
            D = self.D.float()
            A,B,C,D = mask(A,B,C,D, self.pc_mask)
            ...
            mask_larger_matrix(y, self.pc_mask)
            out = self.out_proj(y)
        return out
...
class EncoderLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        mamba_out = self.mamba.forward(emb)
...
class ECCM(torch.nn.Module):
    ...
    def loss(self, z_pred, z2, y):
        loss = F.binary_cross_entropy(
            F.sigmoid(z_pred), sign_to_bin(torch.sign(z2)))
